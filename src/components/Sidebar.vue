      ...
      <v-list>
        <v-list-item v-for="item in sliders" :key="item.name">
          <v-list-item-content class="pa-4">
            ...
            <v-slider v-model="item.value" :step="item.step" :max="item.max" :min="item.min" hide-details>
        ...
        <v-list-item v-for="item in selects" :key="item.name">
          <v-select v-model="item.value" :items="item.values" :label="item.name" class="pa-4"></v-select>
        </v-list-item>
        <v-list-item v-for="item in switches" :key="item.name">
          <v-switch class="pl-4" v-model="item.value" :label="item.name"></v-switch>
        </v-list-item>
      </v-list>
...
export default {
  ...
  data() {
    return {
      ...
      "genetic":{
        variables:[
          { name: "Generation ceiling", value: 100, min: 10, max: 1000, type:"slider" },
  ...
  computed:{
    ...
    selects(){
      return this[this.algo].variables.filter(variable => variable.type=="select")
    },
    sliders(){
      return this[this.algo].variables.filter(variable => variable.type=="slider")
    },
    switches(){
      return this[this.algo].variables.filter(variable => variable.type=="switch")
    },
  },
  methods: {
   run() {
      this.loading = true
      this.$emit('run', this[this.algo].variables.concat({name:"Algorithm", value:this.algo}))
   }
  }
};
</script>